...
wetland.hydrology<-function(giw.INFO, land.INFO, lumped.INFO, snowmelt.VAR, precip.VAR, pet.VAR, n.years, area, volume, giw.ID){
  ...
  upland.FUN<-function(day){
    ...
    # 6a. Variable Moisture Zone Components ---------------------------------
    # NEED TO RENAME/MAKE VARIABLES
    ET_lm.VAR[day,"land"] <<- pet.VAR[day]*(s_lim.VAR[day,"land"]-land.INFO[,"s_wilt"])/
                                  (land.INFO[,"s_fc"]-land.INFO[,"s_wilt"])
    #Recharge to VMZ
    if(s_ex.VAR[day,"land"] < land.INFO[,"s_fc"] ){   # below field capacity
        if(precip.VAR[day]>land.INFO[,"n"]*abs(y_sat.VAR[day,"land"])*
                            (land.INFO[,"s_fc"] - s_ex.VAR[day,"land"] )){ # case where there's more rain than storage
          R_lm.VAR[day,"land"] <<- land.INFO[,"n"]*abs(y_sat.VAR[day,"land"])*(land.INFO[,"s_fc"] - s_ex.VAR[day,"land"])
          R.VAR[day,"land"]    <<- precip.VAR[day] - R_lm.VAR[day,"land"]                                               
        }else{  # less rain that storage
          R_lm.VAR[day,"land"] <<- precip.VAR[day]
          R.VAR[day,"land"]    <<- 0
        }
    }else{ # at field capacity
      R_lm.VAR[day,"land"] <<- 0
      R.VAR[day,"land"]    <<- precip.VAR[day]
    }

    #Loss #### ASSUME ZERO NOW BECAUSE WE ACCOUNT FOR IT IN THE R_lm, and R
    loss_lm.VAR[day,"land"]<<-land.INFO[,"n"]*(s_ex.VAR[day,"land"]-land.INFO[,"s_fc"])*abs(y_sat.VAR[day,"land"])
    loss_lm.VAR[day,"land"]<<- 0*loss_lm.VAR[day,"land"] # CLEAN IT UP NATE...OR FRED
    ...
    y_hm.VAR[day,"land"] <<- 0 # JUST ZEROED TO AVOID ANY WEIRD DEPENDENCY ERRORS; WILL WANT TO CLEAN UP LATER
   
    #Calculate Change in Soil Moisture~~~~~~~~~~~~~~~~~~~~~~~~~
    #Calculate soil moisture
    ds.VAR[day,"land"]<<-ifelse(abs(y_sat.VAR[day,"land"])>0,
                                (R_lm.VAR[day,"land"]-ET_lm.VAR[day,"land"]-loss_lm.VAR[day,"land"])/abs(y_sat.VAR[day,"land"]),
                                0)
    ...
    # 6b. Water Table Components ------------------------------------------
    # Water Table Components
    ET_wt.VAR[day,"land"]<<- min(pet.VAR[day]*exp(y_sat.VAR[day,"land"]/abs(land.INFO[,"RD"])),
                                 ET_lm.VAR[day,"land"])  # ACTUALLY ET_HMZ, IE EXFILTRATION !!!!!!!!!!! HI NATE :) 
    ...
    dy_wt.VAR[day,"land"]<<-(1/land.INFO[,"Sy"])*(R.VAR[day,"land"]+
                                                    ...
                                                    ET_wt.VAR[day,"land"]+ # ACTUALLY ET_HMZ, IE EXFILTRATION
                                                    GW_bf.VAR[day,"land"]-
