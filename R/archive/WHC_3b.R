...
wetland.hydrology<-function(giw.INFO, land.INFO, lumped.INFO, precip.VAR, pet.VAR, n.years, area, volume, giw.ID){
  ...
  upland.FUN<-function(day){
    ...
    # Characterize the vadose zone in the upland -----------------------------
    y_sat.VAR[day,"land"] <<- y_wt.VAR[day,"land"] + abs(land.INFO[,"psi"])
    
    # 6a. ET & Recharge Components ------------------------------------------
    if(y_sat.VAR[day,"land"] < land.INFO[,"y_c"] - 0.01){   # Case 1: Deep water table (LMZ forms)
                                                            # - 0.01 for weird edge case 
      #6a.1 High moistue zone calculations
      psi_fc <-  land.INFO[,"psi"] * land.INFO[,"s_fc"]^(-1/4.05)
      
      if(y_sat.VAR[day,"land"]<= 1*(-5*land.INFO[,"RD"] - psi_fc + land.INFO[,"psi"])){ # condition 3 in eqn 26 in Laio
        y_hm.VAR[day,"land"] <- y_sat.VAR[day,"land"] + psi_fc - land.INFO[,"psi"] 
      }else{
        A <- (psi_fc-land.INFO[,"psi"]-land.INFO[,"y_c"])/
             (psi_fc-land.INFO[,"psi"]-land.INFO[,"y_c"]-(5*abs(land.INFO[,"RD"])))
        y_hm.VAR[day,"land"] <- (1-A^(3/4))*(y_sat.VAR[day,"land"]-land.INFO[,"y_c"])-
                                (A^2*(1-A^(-1/4)))/(-land.INFO[,"y_c"]+psi_fc-land.INFO[,"psi"])*
                                (y_sat.VAR[day,"land"]-land.INFO[,"y_c"])^2
      }
      
      ET_wt.VAR[day,"land"]<<- pet.VAR[day]*exp(y_hm.VAR[day,"land"]/abs(land.INFO[,"RD"])) #Equation 5 McLaughlin et al 2014
      R.VAR[day,"land"] <<- 0
      
      #6a.2 Upland LMZ dynamics ---------------------------------------------
      lmz_stor <- max(0, land.INFO[,"n"]*abs(y_hm.VAR[day,"land"])*(1- s_ex.VAR[day, "land"] ))
      ...
      loss_lm.VAR[day,"land"] <<-max(0, land.INFO[,"n"]*
                                       (s_ex.VAR[day,"land"]-land.INFO[,"s_fc"])*
                                       abs(y_hm.VAR[day,"land"]))
      ExfilSat <- pet.VAR[day] * ( exp(y_hm.VAR[day,"land"] / land.INFO[,"RD"]) -
                                     y_sat.VAR[day,"land"] / land.INFO[,"RD"])         # Exfiltration from water table 
      
      # Special Case: no water left
      if (y_wt.VAR[day,"land"] <= land.INFO[, 'y_cl']){
        ET_wt.VAR[day,"land"] <<- 0;
    ...
    }else{ # Case 2: Shallow water table (only saturated zone)
      y_hm.VAR[day,"land"]<<-0
      ET_wt.VAR[day,"land"] <<- pet.VAR[day]
      PET_lm <- 0
      ExfilSat <- 0
    }
    ...
    ds.VAR[day,"land"]      <<-ifelse(abs(y_hm.VAR[day,"land"])>0,
                                      (R_lm.VAR[day,"land"]-ET_lm.VAR[day,"land"]-
                                         loss_lm.VAR[day,"land"] + ExfilSat)/abs(y_hm.VAR[day,"land"])/land.INFO[,"n"],
                                      0)
  ...
  ####################################################################################
  # Step 7: Model wetland/upland hydrology--------------------------------------------
  ####################################################################################
