...
levels <- subset(rawdata, scale == 'weltand' & (var == 'y_w' | var == 'gw_local' |
                                                var == 'spill_out'  | var == 'runoff_in'))   # need to fix typo!
...
ggplot() + 
  ...
  labs(x = 'Day of Year', title = 'Figure 4a - Delmarva') +
  facet_wrap(vars(var), 
             nrow = 2, 
             scales = 'free_y',
             strip.position = "left", 
...
ggplot() + 
  ...
  theme(plot.title    = element_text(size = 16, face = 'bold'),
        ...
        strip.placement = "outside",
        strip.text    = element_text(size = 12, face = 'bold')) +
  labs(x = 'Day of Year', title = 'Figure 4b - PPR') +
  facet_wrap(vars(var), 
             nrow = 2, 
             scales = 'free_y',
             strip.position = "left", 
             labeller = as_labeller(c(y_w = "Wetland Stage (UNIT)", spill_out = "Spill Out (UNIT)",
                                      gw_local = "Groundwater Exchange (UNIT)", runoff_in = "Runoff In (UNIT)") ))
